...
ALERT_RE = re.compile(r'^\[!(?P<name>NOTE|TIP|IMPORTANT|WARNING|CAUTION)](?:[ \t]*(?:\r?\n|$))', re.I)
ALERT_TITLES = {
    'note': 'Note',
    'tip': 'Tip',
    'important': 'Important',
...
class GithubAlertBlockquote(BlockRule):
    ...
    def parse(self, parser: Parser, state: BlockState, match: re.Match[str]) -> BlockquoteNode | GithubAlertNode:
        ...
        if state.depth == 0:
            alert = parse_alert(text, source_map)
...
def parse_alert(text: str, source: SourceMap | None) -> tuple[str, str, SourceMap | None] | None:
...
def render_github_html_alert(renderer: HTMLRenderer, node: GithubAlertNode, context: HTMLRenderContext) -> str:
...
def render_admonition_html_alert(renderer: HTMLRenderer, node: GithubAlertNode, context: HTMLRenderContext) -> str:
    name = renderer.escape_html(node.name)
    title = renderer.escape_html(ALERT_TITLES[node.name])
    return (
...
class GithubAlertPlugin:
    ...
    def setup(self, wen: Wenmode, /) -> None:
        ...
        if isinstance(rule, Blockquote):
            replacement = GithubAlertBlockquote()
        ...
        if self.html_style == 'github':
            wen.register_renderer_handlers(handlers)
        elif self.html_style == 'admonition':
            wen.register_renderer_handlers({
                **handlers,
                'html': {GithubAlertNode.type: render_admonition_html_alert},
            })


def configure(*, html_style: HtmlStyle = 'github') -> GithubAlertPlugin:
    validate_html_style(html_style)
    return GithubAlertPlugin(html_style=html_style)
